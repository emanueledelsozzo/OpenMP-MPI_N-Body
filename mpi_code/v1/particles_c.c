...
void ComptPopulation(int start, int end, double* weight, struct xy* pos, struct xy* vel, double *forces)
{
    /*
    ...
    */
    ...
    #pragma omp parallel for private(i) \
    shared(start, end, pos, vel, weight, forces, TimeBit)
    for ( i = start; i < end; i++ ) {
        //x0 = p->x[i]; y0 = p->y[i];

        pos[i].x = pos[i].x + (vel[i].x*TimeBit) +
                   (0.5*forces[index2D(0,i,2)]*TimeBit*TimeBit/weight[i]);
        vel[i].x = vel[i].x + forces[index2D(0,i,2)]*TimeBit/weight[i];

        pos[i].y = pos[i].y + (vel[i].y*TimeBit) +
                   (0.5*forces[index2D(1,i,2)]*TimeBit*TimeBit/weight[i]);
...
void SystemEvolution(struct i2dGrid *pgrid, int np, double* weight, struct xy* pos, struct xy* vel, int mxiter, int rank, int size)
{
    ...
    for ( t=0; t < mxiter; t++ ) {
        ...
        ComptPopulation(start, end, weight, pos, vel, forces);
        t0_comm = get_time();
        MPI_Allgatherv (pos + start, send, MPI_LONG_DOUBLE, pos, recv_counts, displs, MPI_LONG_DOUBLE, MPI_COMM_WORLD);
        MPI_Allgatherv (vel + start, send, MPI_LONG_DOUBLE, vel, recv_counts, displs, MPI_LONG_DOUBLE, MPI_COMM_WORLD);
